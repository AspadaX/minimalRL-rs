...
pub fn train<O: SimpleOptimizer<B::InnerBackend>, B: AutodiffBackend>(
    ...
    for _ in 0..10 {
        ...
        let target_network_output: Tensor<B, 2> =
            target_network.forward(data_batch.next_states).max_dim(1);
        // .select(0, Tensor::from_data([0], device))
        // .unsqueeze_dim(1); // DIVERGENCE: differ from the original python code

        let target: Tensor<B, 2> =
            data_batch.rewards + GAMMA * target_network_output * data_batch.dones;
        ...
        let gradients_params: GradientsParams =
            GradientsParams::from_grads(gradients, &q_net_optimized);
...
pub fn run_session() -> Result<()> {
    ...
    let mut optimizer: OptimizerAdaptor<Adam, QNet<Autodiff<NdArray>>, Autodiff<NdArray>> =
        AdamConfig::new().init();
    ...
    for n_epi in 0..10000 {
        ...
        while !done {
            ...
            memory.put(Data::from_step_result(
                previous_state,
                result.observation,
                action as u8,
